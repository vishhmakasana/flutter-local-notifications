...
public class FlutterLocalNotificationsPlugin
    ...
    private static void setSmallIcon(
            ...
            NotificationCompat.Builder builder) {
        try {
            int resIdNotificationIcon = getDrawableResourceId(context, "ic_notification_icon");
            if (resIdNotificationIcon > 0) {
                builder.setSmallIcon(resIdNotificationIcon);
            } else {
                builder.setSmallIcon(android.R.color.white);
            }

            if (!StringUtils.isNullOrEmpty(notificationDetails.icon)) {
                int resId = getDrawableResourceId(context, notificationDetails.icon);
                if (resId > 0) {
                ...
                if (!StringUtils.isNullOrEmpty(defaultIcon)) {
                    int resId = getDrawableResourceId(context, defaultIcon);
                    if (resId > 0) {
                        builder.setSmallIcon(resId);
                    }
                }
